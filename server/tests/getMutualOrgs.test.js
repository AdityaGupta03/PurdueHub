...
describe("Testing getMutualOrgs API:", () => {
  ...
  it("should return 500 if getMutualFriendsQuery fails", async () => {
    ...
    jest.spyOn(accountQueries, "getMutualOrgsQuery").mockReturnValue(null);
  ...
  it("should return 200 if everything is good and no mutual friends", async () => {
    jest.spyOn(accountQueries, "getUserInfoFromUsernameQuery").mockReturnValue({ user_id: 1 });
    jest.spyOn(accountQueries, "getMutualOrgsQuery").mockReturnValue({ rows: [ {mutual_orgs: []} ]});
    ...
    expect(res.body.mutual_orgs).toEqual([]);
  });

  it("should return 200 if everything is good and there are mutual friends", async () => {
    jest.spyOn(accountQueries, "getUserInfoFromUsernameQuery").mockReturnValue({ user_id: 1 });
    jest.spyOn(accountQueries, "getMutualOrgsQuery").mockReturnValue({ rows: [ {mutual_orgs: ["org1"]} ]});
    ...
    expect(res.statusCode).toEqual(200);
    expect(res.body.mutual_orgs).toEqual(["org1"]);
  });
});
