...
describe("Testing favoriting a course API:", () => {
  ...
  it("should return 500 if internal server error", async () => {
    ...
    jest.spyOn(accountQueries, "favoriteCourseQuery").mockReturnValue(false);
    const res = await request(app)
      .post("/api/fav_course")
  ...
  it("should return 200 if everything is good", async () => {
    jest.spyOn(courseQueries, "getCourseID").mockReturnValue(1);
    jest.spyOn(accountQueries, "favoriteCourseQuery").mockReturnValue(true);
    const res = await request(app)
      .post("/api/fav_course")
      .send({ user_id: "1", course_name: "Testing Course" });
