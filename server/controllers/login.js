...
const storage = multer.diskStorage({
	...
	filename: (req, file, cb) => {
	  ...
	  var saveToDatabase = user_id + path.extname(file.originalname);
	},
  });
  ...
  app.post('/upload', upload.single('image'), (req, res) => {
	...
	connection.query('UPDATE accounts SET pic = ? WHERE username = ?', [pathInDB, request.session.username], (error, results, fields) => {
		if (error) throw error;
		else {
			response.send('Profile updated succesfully');
		}
